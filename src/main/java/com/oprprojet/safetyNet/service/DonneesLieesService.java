...
public class DonneesLieesService {
	...
	/**
	 * FireStationStationNumber Cette url doit retourner une liste des personnes
	 * couvertes par la caserne de pompiers correspondante. Donc, si le numéro de
	 * station = 1, elle doit renvoyer les habitants couverts par la station numéro
	 * 1. La liste doit inclure les informations spécifiques suivantes : prénom,
	 * nom, adresse, numéro de téléphone. De plus, elle doit fournir un décompte du
	...
	public Map<String, Object> getFireStationStationNumber(int stationNumber) throws Exception {
		...
		for (Map.Entry<FireStation, List<Person>> entry : donneesEntree.getPersonByFirestation().entrySet()) {
			...
			if (fireStationKey.getStation() != stationNumber) { 
				// si la stationNumber n'est pas égale à celle passée en paramètre je continue
				continue;
			}
			for (Map.Entry<Person, List<MedicalRecord>> entryBoucleDeux : donneesEntree.getMedicalRecordByPerson()
					.entrySet()) {
				Person personKey = entryBoucleDeux.getKey();
				if (!personKey.getAddress().equals(fireStationKey.getAddress())) {
				// si l'adresse de la personne n'est pas identique à celle passée en paramètres on continue de parcourir les éléments
					continue;
				}
				MedicalRecord medicalRecordValue = entryBoucleDeux.getValue().get(0);
				Map<String, String> dataPerson = new HashMap<>(); 
				// ici j'insère les champs dont j'ai besoin dans une map clé, valeur
				...
				if (medicalRecordValue.isAdulte()) {
					// ce if sert a déterminer qui a + de 18 ans et l'affecter dans la bonne liste.
	...
	/**
	 * ChildAlertAddress Retourne une liste d'enfants (tout individu âgé de 18 ans
	 * ou moins) habitant à l'adresse passée en paramètres. La liste doit comprendre
	 * le prénom et le nom de famille de chaque enfant, son âge et une liste des
	 * autres membres du foyer. S'il n'y a pas d'enfant, cette url peut renvoyer une
	 * chaîne vide.
	 * 
	 * @param address
	 * @return
	...
	public Map<String, Object> getChildAlertAddress(String address) throws Exception {
		...
		/* pour chaque key (person) de la Map si l'adresse est égale à l'adresse passée
		 en paramètre, alors la Person est ajoutée dans la liste Adulte
	...
	/**
	 * PhoneAlertFireStation Cette url doit retourner une liste des numéros de
	 * téléphone des résidents desservis par la caserne de pompiers. Nous
	 * l'utiliserons pour envoyer des messages texte d'urgence à des foyers
	 * spécifiques.
	 * 
	 * @param stationNumber
	 * @return
	...
	public Map<String, Object> getPhoneAlerteFireStation(int stationNumber) throws Exception {
		...
		for (Map.Entry<FireStation, List<Person>> entry : donneesEntree.getPersonByFirestation().entrySet()) {
			...
			for (Map.Entry<Person, List<MedicalRecord>> entryBoucleDeux : donneesEntree.getMedicalRecordByPerson()
					.entrySet()) {
				Person personKey = entryBoucleDeux.getKey();
				if (!personKey.getAddress().equals(fireStationKey.getAddress())) {// si l'adresse de la personne n'est
																					// pas identique à celle passée en
																					// paramètres on continue de
	...
	/**
	 * FireAddress Cette url doit retourner la liste des habitants vivant à
	 * l’adresse donnée ainsi que le numéro de la caserne de pompiers la desservant.
	 * La liste doit inclure le nom, le numéro de téléphone, l'âge et les
	...
	/**
	 * FloodStation Cette url doit retourner une liste de tous les foyers desservis
	 * par la caserne. Cette liste doit regrouper les personnes par adresse. Elle
	 * doit aussi inclure le nom, le numéro de téléphone et l'âge des habitants, et
	...
	/**
	 * personInfoLastName Cette url doit retourner le nom, l'adresse, l'âge,
	 * l'adresse mail et les antécédents médicaux (médicaments, posologie et
	 * allergies) de chaque habitant. Si plusieurs personnes portent le même nom,
	 * elles doivent toutes apparaître.
	 * 
	 * @throws Exception
	 * 
	 */
	public Map<String, Object> getPersonInfoLastName(String lastName) throws Exception {
		...
		for (Map.Entry<Person, List<MedicalRecord>> entry : donneesEntree.getMedicalRecordByPerson().entrySet()) {
			Person person = entry.getKey();
			if (!person.getLastName().equals(lastName)) {// si l'adresse de la personne n'est pas identique à celle
															// passée en paramètres on continue de parcourir les
															// éléments
				continue;
			}
			MedicalRecord medicalRecord = entry.getValue().get(0);
	...
	/**
	 * CommunityEmail retourne la liste des adresses mail des personnes habitants
	...
	public Map<String, Object> getCommunityEmail(String city) throws Exception {
		...
		for (Map.Entry<Person, List<MedicalRecord>> entry : donneesEntree.getMedicalRecordByPerson().entrySet()) {
			...
			if (!personKey.getCity().equals(city)) {// si l'adresse de la personne n'est pas identique à celle passée en
													// paramètres on continue de parcourir les éléments
				continue;
			}
			Map<String, String> dataPerson = new HashMap<>(); // ici j'insère les champs dont j'ai besoin dans une map  clé, valeur
	...
	/**
	 * méthode qui gère la lecture et la transformation des données brutes en
	 * données liées.
	 * 
	 * @return
	 * @throws Exception
	...
	/**
	 * Retourne une Map<String, Object> triant les personnes par addresse et
	 * indiquant à quelle stationNumber elles sont rattachées
	 * 
	 * @param address
	 * @param donneesEntree
	 * @return
	...
	private Map<String, Object> getMapPersonnesParAdresse(String address, DonneesLiees donneesEntree) {
		List<Map<String, Object>> listPerson = new ArrayList<>(); // je fais une map String Object, car les médicaments
																	// et allergies sont des listes de String. Je ne
																	// pourrai donc pas stocker dans la même map les
																	// médicaments et l'age par exemple
		Integer stationNumber = 0;
		for (Map.Entry<FireStation, List<Person>> entry : donneesEntree.getPersonByFirestation().entrySet()) {
			FireStation fireStationKey = entry.getKey();
			if (!fireStationKey.getAddress().equals(address)) { // si la stationNumber n'est pas égale à celle passée en
																// paramètre je continue
				continue;
			}
			...
			for (Map.Entry<Person, List<MedicalRecord>> entryBoucleDeux : donneesEntree.getMedicalRecordByPerson()
				...
				if (!person.getAddress().equals(address)) {// si l'adresse de la personne n'est pas identique à celle
															// passée en paramètres on continue de parcourir les
															// éléments
					continue;
				}
