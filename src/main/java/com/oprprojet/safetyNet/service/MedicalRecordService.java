...
public class MedicalRecordService {
	...
	public MedicalRecord addMedicalRecord(MedicalRecord medicalRecord) throws Exception {
		...
		// Ajouter le medicalRecord transmise dedans en vérifiant qu'elle n'existe pas
		// déjà.
		...
		for (MedicalRecord medicalRecordElement : medicalRecordList) {
			if (medicalRecordElement == medicalRecord) {
				...
				logger.warn(
						"methode addMedicalRecord : le MedicalRecord que vous essayez d'ajouter existe déjà, il ne sera pas ajouté");
				break;
	...
	public void deleteMedicalRecord(String firstName, String lastName) throws Exception {
		...
		List<MedicalRecord> medicalRecordList = donneesBrute.getMedicalRecords();
		for (MedicalRecord medicalRecordElement : medicalRecordList) {
			if (medicalRecordElement.getFirstName().equals(firstName)
					&& medicalRecordElement.getLastName().equals(lastName)) {
	...
	public MedicalRecord updateMedicalRecord(MedicalRecord medicalRecord) throws Exception {
		...
		List<MedicalRecord> medicalRecordList = donneesBrute.getMedicalRecords();
		for (MedicalRecord medicalRecordElement : medicalRecordList) {
			if (!medicalRecordElement.getFirstName().equals(medicalRecord.getFirstName())
					&& !medicalRecordElement.getLastName().equals(medicalRecord.getLastName())) {
				continue;
			}
			// modifie birthdate / allergies / medications uniquement si ces infos sont
