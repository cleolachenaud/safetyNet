...
public class MedicalRecordServiceTest {
	...
	public void setUp() throws Exception {
		...
		MedicalRecord medicalRecord1 = new MedicalRecord("Appa", "Paddaone", birthdate, List.of("baytril"),
				List.of("avocat"));
	...
	public void addMedicalRecordTest() throws Exception {
		...
		MedicalRecord medicalRecord3 = new MedicalRecord("Marley", "Paddathree", birthdate, List.of(""),
				List.of("avocat"));
		try {
			medicalRecordService.addMedicalRecord(
					new MedicalRecord("Marley", "Paddathree", birthdate, List.of(""), List.of("avocat")));
		} catch (Exception e) {
		...
		assertEquals("la liste de données doit être égale à 3", 3,
				argumentCaptorDonnees.getValue().getMedicalRecords().size());
		assertEquals("les éléments de la variable station doivent être identiques à ceux capturées par le mock",
				medicalRecord3, argumentCaptorDonnees.getValue().getMedicalRecords().get(2));
	}
	...
	public void updateMedicalRecordTest() throws Exception {
		...
		try {
			medicalRecordService.updateMedicalRecord(
					new MedicalRecord("Appa", "Paddaone", birthdate, List.of("baytril"), List.of("avocat")));
		} catch (Exception e) {
			erreurLoggee = e.toString();
		}
		...
		assertEquals("la liste de données doit être égale à 2", 2,
				argumentCaptorDonnees.getValue().getMedicalRecords().size());
		assertEquals("les éléments de la variable station doivent être identiques à ceux capturées par le mock",
				medicalRecord1, argumentCaptorDonnees.getValue().getMedicalRecords().get(0));
	}
