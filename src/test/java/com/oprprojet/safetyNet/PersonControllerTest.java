...
public class PersonControllerTest {
    ...
    public void addPersonTest() throws Exception {
    	person = new Person("Appa", "Paddaone", "36 rue des paddas", "paddaVille", 37100, "841-874-7462", "appa@email.com");
       doReturn(person).when(personService).addPerson(any());
		mockMvc.perform(
		   	post("http://localhost:8080/person", person)
    ...
    public void majPersonTest() throws Exception {
    	person = new Person("Moja", "Paddaone", "36 rue des paddas", "paddaVille", 37100, "841-874-7462", "appa@email.com");
    doReturn(person).when(personService).updatePerson(any());
	   	mockMvc.perform(
		   	put("http://localhost:8080/person", person)
            .contentType(MediaType.APPLICATION_JSON)
            .characterEncoding("utf-8")
            .content(jsonContent)
		).andExpect(status().isOk())
